...
class One_Form_Reader_Xml implements One_Form_Reader_Interface
{
	 ...
	 */
	public static function load($scheme, $formFile = 'form', $language = NULL, $formName = 'oneForm', $action = '', $method = 'post')
	{
    ...
    $validFile = self::$_dom->loadXML($parsedContent);
		...
		if($validFile !== false)
		{
			...
			{
				One::getInstance()->setLanguage($oriLang); // Reset the original language
	...
	private static function addObligatedWidgets(One_Form_Container_Form $form, One_Scheme $scheme)
	{
		...
		if(!is_null($scheme->getIdentityAttribute()) && !$form->hasWidget($scheme->getIdentityAttribute()->getName())) {
			$form->addWidget(new One_Form_Widget_Scalar_Hidden($scheme->getIdentityAttribute()->getName(), $scheme->getIdentityAttribute()->getName(), NULL, array('one' => 'one', 'language' => strtolower(One::getInstance()->getLanguage()))));
		}
		if(!$form->hasWidget('task')) {
			$form->addWidget(new One_Form_Widget_Scalar_Hidden('task', 'task', NULL, array('default' => 'edit', 'one' => 'one', 'language' => strtolower(One::getInstance()->getLanguage()))));
		}
		if(!$form->hasWidget('scheme')) {
			$form->addWidget(new One_Form_Widget_Scalar_Hidden('scheme', 'scheme', NULL, array('default' => $scheme->getName(), 'one' => 'one', 'language' => strtolower(One::getInstance()->getLanguage()))));
		}
		// TR 20110921 commented action as it is no longer obligated, for the few that still need it, it should be put manually
//		if(!$form->hasWidget('action')) {
//			$form->addWidget(new One_Form_Widget_Scalar_Hidden('action', 'action', NULL, array('default' => 'cancel', 'one' => 'one')));
//		}
	}
	...
	protected static function _parseToForm(DOMElement $element, One_Form_Container_Abstract $container)
	{
		...
		if(preg_match('/^One_Form_Container/', $widgetClass) && $widgetClass != 'One_Form_Container_Form')
		{
			...
			if(false === isset($attributes['language'])) {
				$attributes['language'] = strtolower(One::getInstance()->getLanguage());
			}
		...
		else
		{
			if($element->hasAttribute('role'))
			{
				...
				if($element->hasAttribute('optionsFrom')) // set more complex options by getting a json-string from a view
				{
					...
					{
						$relView = new One_View($relScheme->getName(), $element->getAttribute('optionsFrom'), strtolower(substr(One::getInstance()->getLanguage(), 0, 5)));
				...
				else
				{
					...
					if(false === $element->hasAttribute('cacheOptions') || ($element->hasAttribute('cacheOptions') && false === $session->varExists($sessionCacheName, 'One_Form_Cache')))
					{
						...
						if($element->hasAttribute('publishedOnly'))
						{
							$publishOptions = $relScheme->getBehaviorOptions('published');
							if(is_array($publishOptions) && isset($publishOptions['attribute'])) {
								$q->where($publishOptions['attribute'], 'eq', 1);
							}
						}
	...
	protected static function determineWidget($container, $element, $widgetClass)
	{
		...
		if($element->hasAttribute('optionsFrom') && method_exists($widget, 'setOptions')) // set more complex options by getting a json-string from a view
		{
			...
			if(2 == count($parts))
			{
				...
				if(false === $element->hasAttribute('cacheOptions') || ($element->hasAttribute('cacheOptions') && false === $session->varExists($sessionCacheName, 'One_Form_Cache')))
				{
					$optView = new One_View($parts[0], $parts[1], strtolower(substr(One::getInstance()->getLanguage(), 0, 5)));
					try
					{
	...
	{
		$filename = One::getInstance()->getCustomPath().'/conditions/'.strtolower($include).'.xml';
