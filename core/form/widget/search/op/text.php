<?php
/**
 * Handles search option for strings
 *
 * @author delius
 * @copyright 2010 delius bvba
 * @package one|content
 * @filesource one/lib/form/widget/search/op/text.php
 * @license http://www.gnu.org/licenses/gpl-2.0.html GNU/GPL
 **/
class One_Form_Widget_Search_Op_Text extends One_Form_Widget_Search_Op_Abstract
{
	/**
	 * Class constructor
	 *
	 * @param One_Form_Widget_Abstract $forWidget
	 */
	public function __construct( $forWidget )
	{
		$this->forWidget = $forWidget;
		$this->type      = 'text';
	}

	/**
	 * Render the op
	 */
	public function render()
	{
		$data = array(
						'for'   => $this->forWidget,
						'value' => $this->getValue()
					);

		$content = $this->parse( $data );
		return $content;
	}

	/**
	 * Affect the One_Query appropriately
	 *
	 * @param One_Query $q
	 * @param string $attrName
	 * @param mixed $attrVal
	 * @final
	 */
	final public function affectQuery( One_Query $q, $attrName, $attrVal )
	{
		$allowed = self::getAllowed();
		$opVal   = $this->getValue();

		if(!is_null($opVal))
		{
			if( in_array( $opVal, $allowed ) )
				$op = $opVal;
			else
				$op = $allowed[ 'default' ];
		}
		else
			$op = $allowed[ 'default' ];

		$q->where( $attrName, $op, $attrVal );
	}

	/**
	 * Get the allowed operators
	 *
	 * @return array
	 * @final
	 */
	final public function getAllowed()
	{
		return array(
						'default' => 'contains',
						'eq',
						'contains',
						'begins',
						'ends'
					);
	}
}