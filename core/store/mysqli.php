<?php
   ...
   * ONEDISCLAIMER
   **/
  class One_Store_Mysqli extends One_Store
  {
    ...
    public function insert(One_Model $model)
    {
      ...
      if ($result === false) {
        throw new One_Exception('MYSQLI INSERT error (' . $db->connect_errno . ') '. $db->connect_error);
      }
    ...
    public function update(One_Model $model)
    {
      ...
      if (count($clauses) > 0) {
        $result = $db->query($sql);
        if ($result === false) {
          throw new One_Exception('MYSQLI UPDATE error (' . $db->connect_errno . ') '. $db->connect_error);
        }
      }
    ...
    public function setEncoding(One_Scheme $scheme, $encoding)
    {
      ...
      if ($result === false) {
        throw new One_Exception('MYSQLI error (' . $db->connect_errno . ') '. $db->connect_error . '<br />' . $sql);
      }
    }
  }
