...
/**
 * Defines how OCI should be addressed to perform certain storage and retrieval actions
 *
 * @author delius
...
class One_Store_Oci implements One_Store_Interface
{
	/**
	...
	/**
	 * @var object OCI database object used
	 */
	protected $dbh = NULL;
	...
	protected function getRenderer()
	{
		...
		return One_Query_Renderer::getInstance('oci');
	}
...
//DBG
list($us,$s) = explode(" ",microtime());
$parse = (float) $us + (float) $s;
//DBG

//DBG
//die($stid);
//echo '<br>Parsed <b>', $sql, '</b> into ', $stid;
//DBG
...
//DBG
//echo '<br/><span style="color: blue;">parse=' . ($parse-$start) . ', exec = ' . ($ex - $parse) . '</span>';
//echo '<br>Executed it : ';
//DBG
...
//DBG
	...
	public function saveRelations(One_Model_Interface $model, One_Link_Interface $link)
	{
		...
		if (isset($modified[$link->getName()]))
		{
			...
			if(!($linkConnection->getStore() instanceof One_Store_Oci)) {
	...
	public function update(One_Model_Interface $model)
	{
		...
		foreach($scheme->getAttributes() as $attName => $at)
		{
			if(isset($modified[$attName]))
			{
				$clauses[] = One_Query_Renderer::getInstance('oci')->formatAttribute($at, $modified[$attName]);
			}
		}
